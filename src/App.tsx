...
type ModeType = 'Executive' | 'Technical' | 'Creative';
...
const App = () => {
  const [activeMode, setActiveMode] = useState<ModeType>('Executive');
  const [hoveredAgent, setHoveredAgent] = useState<Agent | null>(null);
  const [pulse, setPulse] = useState(false);

  useEffect(() => {
    const interval = setInterval(() => setPulse((p: boolean) => !p), 2000);
    return () => clearInterval(interval);
  }, []);

  const modes: Record<ModeType, { color: string; bg: string; border: string; icon: LucideIcon; desc: string }> = {
      ...
      <div className="relative w-[600px] h-[600px] flex items-center justify-center">
        ...
        <div className={`relative w-48 h-48 rounded-full flex flex-col items-center justify-center p-6 text-center transition-all duration-700 z-30 border-2 ${modes[activeMode].border} ${modes[activeMode].bg} shadow-2xl`}>
          ...
          <div className="absolute -bottom-16 flex gap-2">
            {(Object.keys(modes) as ModeType[]).map(m => (
              <button
                key={m}
        ...
        {hoveredAgent && (
          <div className="absolute top-0 right-[-260px] w-56 bg-slate-900/95 backdrop-blur-md border border-emerald-400/50 p-4 rounded-xl shadow-2xl z-40 animate-in fade-in slide-in-from-left-4">
